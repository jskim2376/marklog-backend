...
public class UserService {
	...
	public Long update(Long id, PostUpdateRequestDto requestDto) {
		Users user = usersRepository.findById(id).orElseThrow(()->new IllegalArgumentException("해당 게시글이 없습니다. id="+id));
		user.update(requestDto.getTitle(), requestDto.getContent());
		return id;
	}
	
	public UserResponseDto findById(Long id) {
		Users entity = usersRepository.findById(id).orElseThrow(()->new IllegalArgumentException("해당 게시글이 없습니다. id="+id));
